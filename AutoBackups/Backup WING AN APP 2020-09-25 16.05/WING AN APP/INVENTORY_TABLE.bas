	...
	If LOGIN_MODULE.username <> "" Or LOGIN_MODULE.tab_id <> "" Then
		...
		Else
			If BUTTON_EDIT.Text = " Edit" Then
				Dim query As String = "UPDATE inventory_disc_table SET inventory_status = ? , recount_by = ? , recount_person1 = ? , recount_person2 = ? WHERE transaction_number = ? AND transaction_id = ?"
				connection.ExecNonQuery2(query,Array As String("RECOUNT",LOGIN_MODULE.username,MONTLHY_INVENTORY2_MODULE.assign1,MONTLHY_INVENTORY2_MODULE.assign2,LABEL_LOAD_NUMBER.Text,MONTHLY_INVENTORY_MODULE.transaction_id))
			...
			Else
				...
				Dim query As String = "UPDATE inventory_disc_table SET unit = ? , quantity = ?, total_pieces = ?, edit_count = ?, inventory_status = ? , recount_by = ? , recount_person1 = ? , recount_person2 = ? WHERE transaction_number = ? AND transaction_id = ?"
				connection.ExecNonQuery2(query,Array As String(CMB_UNIT.cmbBox.SelectedItem,EDITTEXT_QUANTITY.Text,total_pieces,edit_count + 1,"RECOUNT",LOGIN_MODULE.username,MONTLHY_INVENTORY2_MODULE.assign1,MONTLHY_INVENTORY2_MODULE.assign2,LABEL_LOAD_NUMBER.Text,MONTHLY_INVENTORY_MODULE.transaction_id))
				ProgressDialogShow2("Recounting...", False)
				Sleep(500)
				ProgressDialogHide()
				Sleep(0)
...
Sub Insert_Inventory_Disc
	...
	If cursor3.RowCount > 0 Then
		For i3 = 0 To cursor3.RowCount - 1
			...
			cursor3.GetString("date_registered"),cursor3.GetString("time_registered"),cursor3.GetString("edit_count"),cursor3.GetString("tab_id")))
			Dim js As HttpJob = CreateRequest.ExecuteBatch(Array(cmd), Null)
			Wait For(js) JobDone(js As HttpJob)
			If js.Success Then
...
Sub Insert_Inventory_Disc_Trail
	...
	If cursor4.RowCount > 0 Then
		For i4 = 0 To cursor4.RowCount - 1
			...
			cursor4.GetString("date_registered"),cursor4.GetString("time_registered"),cursor4.GetString("edit_count"),cursor4.GetString("tab_id"), _
