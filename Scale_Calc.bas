...
Public Sub Initialize
	If GetDevicePhysicalSize < 6 Then
		If 100%x > 100%y Then
			...
			cScaleY = 100%y / 270dip
		Else
			' portrait
			cScaleX = 100%x / 320dip
			cScaleY = 100%y / 430dip
		End If
	Else
		If 100%x > 100%y Then
			' landscape
			cScaleX = 1 + Rate * (100%x / 480dip - 1)
			cScaleY = 1 + Rate * (100%y / 270dip - 1)
		Else
			' portrait
			cScaleX = 1 + Rate * (100%x / 320dip - 1)
			cScaleY = 1 + Rate * (100%y / 430dip - 1)
		End If
	End If
	cScaleDS = 1 + Rate * ((100%x + 100%y) / (320dip + 430dip) - 1)
End Sub
