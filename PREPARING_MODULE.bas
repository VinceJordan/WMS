		...
		Else
			...
			Else
				If unit_trigger = 1 Then
					Msgbox2Async("The unit " & LABEL_LOAD_SCANCODE.Text &" you scan is not ordered in the picklist, Please check carefully the unit you preparing before you proceed. Would you like to continue?", "Unit not in order!", "YES", "", "NO", LoadBitmap(File.DirAssets, "LOGO_3D.png"), True)
					Wait For Msgbox_Result (Result As Int)
					If Result = DialogResponse.POSITIVE Then
						PREPARED_PCS
					End If
...
Sub NOT_UNIT_TRIGGER
	If LABEL_LOAD_SCANCODE.Text = "PCS" And LABEL_PCS_QTY_PREPARING.Text = "0" Then
		unit_trigger = 1
	Else If LABEL_LOAD_SCANCODE.Text = "CASE" And LABEL_CASE_QTY_PREPARING.Text = "0" Then
		unit_trigger = 1
	Else If LABEL_LOAD_SCANCODE.Text = "BOX" And LABEL_BOX_QTY_PREPARING.Text = "0" Then
		unit_trigger = 1
	Else If LABEL_LOAD_SCANCODE.Text = "DOZ" And LABEL_DOZ_QTY_PREPARING.Text = "0" Then
		unit_trigger = 1
	Else If LABEL_LOAD_SCANCODE.Text = "BAG" And LABEL_BAG_QTY_PREPARING.Text = "0" Then
		unit_trigger = 1
	Else If LABEL_LOAD_SCANCODE.Text = "PACK" And LABEL_PACK_QTY_PREPARING.Text = "0" Then
		unit_trigger = 1
